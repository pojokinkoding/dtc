...
echo "Inspecting all non-empty rows of Sheet 'Torque Tekanan Angin' in ctp_precase_jan.xlsx:\n\n";
foreach ($rows as $rNum => $rowCells) {
    $filtered = array_filter($rowCells, function($v) { return $v !== '' && $v !== null; });
    if (!empty($filtered)) {
        echo "Row $rNum: " . json_encode($filtered, JSON_UNESCAPED_UNICODE) . "\n";
    }
}
