...
// Find "CTP Proses" headers which mark the start of each model block
echo "Scanning for model blocks (rows with 'CTP Proses' or 'Nama CTQ' or 'Line'):\n\n";
foreach ($rows as $r => $cells) {
    $bVal = $cells['B'] ?? '';
    $adVal = $cells['AD'] ?? '';
    if (stristr($bVal, 'CTP') !== false || stristr($bVal, 'Nama CTQ') !== false || 
        stristr($bVal, 'Nama Model') !== false || stristr($adVal, 'REF') !== false ||
        stristr($bVal, 'Jam') !== false) {
