<?php
// Importer for 202601 Check Sheet Cycle COBRA REF01.xlsx with Quantitative / Qualitative Checkpoint Types
require_once __DIR__ . '/../config/config.php';

$file_name = $argv[1] ?? '202601 Check Sheet Cycle COBRA REF01.xlsx';
...
// 1. STEP 1: CLEAN PREVIOUS DATA FOR COBRA / CLAMPING / CHARGING FOR 2026-01
...
$conn->exec("DELETE FROM dtc_master_dtc_specs WHERE line_name = '$line_name' AND section_name IN ('Clamping', 'Charging') AND data_type = '$data_type'");
...
foreach ($sheets as $sh) {
    ...
    // 1. Create master spec entry in dtc_master_dtc_specs
    ...
    // 2. Create master parameter entry in dtc_master_parameters
    ...
    foreach ($checkpoints_def as $cpName => $cpInfo) {
        // Sample cell values across all date columns to determine if checkpoint is Quantitative or Qualitative
        $sampleVals = [];
        ...
        // Determine type: if there are sample values and all non-OK/NG sample values are numeric numbers -> Quantitative, else Qualitative
        $isNumeric = false;
        $numCount = 0;
        ...
        // If at least 50% of recorded values are numeric, classify as Quantitative
...
echo "====================================================\n";
echo "RE-IMPORT COMPLETE SUMMARY:\n";
