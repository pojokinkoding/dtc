<?php
// Importer for CTP PU Case Excel file for June 2026 (ctp_pucase_jun.xlsx)
require_once __DIR__ . '/../config/config.php';

echo "Starting CTP PU Case Import for June 2026 (ctp_pucase_jun.xlsx)...\n";

$file = __DIR__ . '/../ctp_pucase_jun.xlsx';
if (!file_exists($file)) {
    die("Error: File ctp_pucase_jun.xlsx not found.\n");
}
...
for ($idx = 4; $idx < count($sheets); $idx++) {
    ...
    // 2. Find or Insert Parameter in dtc_master_parameters for target_month = '2026-06'
    ...
    if (!$parameter_id) {
        ...
        echo "Created NEW Parameter ID: $parameter_id for June 2026\n";
    } else {
        echo "Found existing Parameter ID: $parameter_id for June 2026\n";
    }

    // 3. Ensure running model entry in dtc_running_models for target_month = '2026-06'
    ...
    foreach ($valid_columns as $col => $day) {
        $inspection_date = sprintf('2026-06-%02d', $day);
...
echo "\n====================================================\n";
echo "CTP PU CASE JUNE 2026 IMPORT COMPLETE SUMMARY:\n";
