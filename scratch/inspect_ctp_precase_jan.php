...
if (!file_exists($file)) die("File ctp_precase_jan.xlsx not found\n");
...
echo "Total sheets in ctp_precase_jan.xlsx: " . count($sheets) . "\n\n";
...
for ($idx = 0; $idx < count($sheets); $idx++) {
    ...
    // Check sample rows with non-empty Column B time labels
    $sample_count = 0;
    $time_slots = [];
    if ($dateRow) {
        $sample_row_indices = [];
        for ($sr = $dateRow + 1; $sr <= $dateRow + 10; $sr++) {
            ...
            if (strcasecmp($lbl, 'Xbar') === 0 || strcasecmp($lbl, 'R') === 0 || stristr($lbl, 'StdDEV') !== false) break;
            if ($lbl !== '') {
