...
// Check the sheets that might have wrong item name
$checkSheets = [4, 20, 21, 22]; // 0-indexed: Reactivity(H1), JIG(OUT), Jig(Core), Pre-Assembly

foreach ($checkSheets as $idx) {
    $s = $sheets[$idx];
    $sheetNum = $idx + 1;
    $sheetName = $s['name'];
