<?php
// Generic Importer for Time Check Excel Files
require_once __DIR__ . '/../config/config.php';

$file_name = $argv[1] ?? 'timecheck_v_forming_male_B_ref01_jan.xlsx';
$target_month = $argv[2] ?? '2026-01';
$line_name = $argv[3] ?? 'REF 01';
$section_name = $argv[4] ?? 'V Forming Male B';
$data_type = $argv[5] ?? 'Time Check';

echo "=== STARTING TIME CHECK IMPORT ===\n";
echo "File: $file_name\n";
...
function getDrawingTextsByRow($zip, $targetMap, $rId) {
    ...
    foreach ($dXml->children('xdr', true) as $anchor) {
        $fromRow = (int)($anchor->from->row ?? 0) + 1;
        $texts = [];
...
for ($idx = 0; $idx < count($sheets); $idx++) {
    ...
    $model_name = $sheetName; // Use sheet name as model name per rule
    ...
    foreach ($valid_columns as $col => $day) {
        $date = sprintf('%s-%02d', $target_month, $day);
        ...
        if (!$sess_id) {
            ...
            $stmt_upd_s = $conn->prepare("UPDATE dtc_inspection_sessions SET is_closed = 1 WHERE session_id = :sid");
            $stmt_upd_s->execute([':sid' => $sess_id]);
        }
...
$zip->close();
echo "\n====================================================\n";
echo "GENERIC TIME CHECK IMPORT COMPLETE SUMMARY:\n";
