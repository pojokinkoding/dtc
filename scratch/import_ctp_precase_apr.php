<?php
// Importer for CTP Pre Case Excel file for April 2026 (ctp_precase_apr.xlsx)
require_once __DIR__ . '/../config/config.php';

echo "Starting CTP Pre Case Import for April 2026 (ctp_precase_apr.xlsx)...\n";

$file = __DIR__ . '/../ctp_precase_apr.xlsx';
if (!file_exists($file)) {
    die("Error: File ctp_precase_apr.xlsx not found.\n");
}
...
function formatPrecaseTimeLabel($bVal, $seq) {
    $bVal = str_replace('.', ':', trim((string)$bVal));
...
for ($idx = 0; $idx < count($sheets); $idx++) {
    ...
    $process_name = isset($rows[6]['E']) ? trim((string)$rows[6]['E']) : 'Hinge Lower Assy';
    if (!$process_name) $process_name = 'Hinge Lower Assy';
    ...
    foreach ($valid_columns as $col => $day) {
        ...
        foreach ($sample_row_indices as $sr) {
            ...
            $lbl = formatPrecaseTimeLabel($raw_b, $seq);
...
echo "\n====================================================\n";
echo "CTP PRE CASE APRIL 2026 IMPORT COMPLETE SUMMARY:\n";
