...
echo "================ COMPREHENSIVE CTP PU DOOR LINE REF 02 RECAP (JAN - JUN 2026) ================\n";
...
foreach ($months as $m) {
    $stmt = $conn->prepare("
        ...
        WHERE p.target_month = :m AND p.data_type = 'CTP' AND p.line_name = 'REF 02' AND p.section_name = 'PU Door'
    ");
    $stmt->execute([':m' => $m]);
