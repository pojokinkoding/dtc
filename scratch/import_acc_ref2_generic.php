<?php
// Importer for CTP Accessories REF 02 - Generic
// Usage: php import_acc_ref2_generic.php <filename.xlsx> <YYYY-MM>
require_once __DIR__ . '/../config/config.php';

if ($argc < 3) die("Usage: php import_acc_ref2_generic.php <filename.xlsx> <YYYY-MM>\n");
...
if (!file_exists($file)) die("Error: File $filename not found.\n");

echo "Starting CTP Accessories Import (REF 02) - $target_month...\n";
echo "Using file: $filename\n\n";

$line_name    = 'REF 02';
$section_name = 'Accessories'; // Corrected spelling (not "Accessoris")
$data_type    = 'CTP';
...
function cleanTimeLabel($lbl) {
    ...
    // Already in HH:MM format or other text
    return $lbl;
}
...
for ($idx = 0; $idx < count($sheets); $idx++) {
    ...
    // ---- Detect Line (check rows 1-3 col AF or AD) ----
    $lineCell = '';
    for ($r = 1; $r <= 4; $r++) {
        foreach (['AF', 'AD', 'AE', 'AG'] as $c) {
            $v = trim($rows[$r][$c] ?? '');
            if (stristr($v, 'REF') !== false || stristr($v, 'Line') !== false) {
                $lineCell = $v; break 2;
            }
        }
    }
    // Only import if it's REF 02
    if ($lineCell !== '' && stristr($lineCell, 'REF 02') === false && stristr($lineCell, 'REF02') === false) {
        echo "Sheet $sheetNum '$sheetName': Line is '$lineCell' (not REF 02) - SKIPPED.\n";
        $summary_results[$sheetName] = ['status' => 'SKIPPED_WRONG_LINE', 'sessions' => 0, 'measurements' => 0];
        continue;
    }
    ...
    // Item check name: look for "Nama CTQ" in col B rows 3-7, value in col E
    $item_check_name = '';
    ...
    for ($r = 1; $r <= 10; $r++) {
        ...
        // Item check name
        if (strpos($bVal, 'nama ctq') !== false) {
            $item_check_name = trim($rows[$r]['E'] ?? '');
        }

        // Process name
        if (strpos($bVal, 'nama proses') !== false) {
            $process_name = trim($rows[$r]['E'] ?? 'Assembly');
            if (!$process_name) $process_name = 'Assembly';
        }

        // LSL / USL from col I (label) + col L (value), col M (UOM)
    ...
    $dateRow = null;
    for ($r = 15; $r <= 30; $r++) {
        if (strpos(strtolower(trim($rows[$r]['B'] ?? '')), 'jam') !== false) {
            $dateRow = $r; break;
        }
    }
    if (!$dateRow) {
    ...
    // Sample rows (after date row, until "Max", "Min", "Average", "Xbar" in col B)
    $sample_rows = [];
    ...
    // Valid day columns (from date row)
    $valid_columns = [];
    ...
    echo "  Model: '$model_name' | Item: '$item_check_name' | Process: '$process_name' | LSL: $lsl | USL: $usl $uom | DateRow: $dateRow | SampleRows: " . count($sample_rows) . " | DataCols: " . count($valid_columns) . "\n";
...
echo "\n====================================================\n";
echo "CTP ACCESSORIES (REF 02) $target_month IMPORT COMPLETE:\n";
