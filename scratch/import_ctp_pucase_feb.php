<?php
// Importer for CTP PU Case Excel file for February 2026 (ctp_pucase_feb.xlsx)
require_once __DIR__ . '/../config/config.php';

echo "Starting CTP PU Case Import for February 2026 (ctp_pucase_feb.xlsx)...\n";

$file = __DIR__ . '/../ctp_pucase_feb.xlsx';
if (!file_exists($file)) {
    die("Error: File ctp_pucase_feb.xlsx not found.\n");
}
...
// Per user instruction: start from sheet 5 (index 4 in array)
for ($idx = 4; $idx < count($sheets); $idx++) {
    ...
    // Line is explicitly REF 01 per user request
    $line_name = 'REF 01';

    // Section is explicitly PU Case
    $section_name = 'PU Case';

    // Data type is explicitly CTP
    $data_type = 'CTP';

    // Extract item_check_name / process_name from header
    $item_check_name = '';
    for ($r = 1; $r <= 6; $r++) {
    ...
    // Per user instruction: model_name is set to sheetName
    ...
    // Detect date header row (row 10..15)
    $dateRow = null;
    for ($r = 10; $r <= 15; $r++) {
        if (isset($rows[$r])) {
    ...
    // Find sample rows following dateRow
    $sample_row_indices = [];
    ...
    // 2. Find or Insert Parameter in dtc_master_parameters for target_month = '2026-02'
    ...
    if (!$parameter_id) {
        ...
        echo "Created NEW Parameter ID: $parameter_id for Feb 2026\n";
    } else {
        echo "Found existing Parameter ID: $parameter_id for Feb 2026\n";
    }

    // 3. Ensure running model entry in dtc_running_models for target_month = '2026-02'
    ...
    foreach ($valid_columns as $col => $day) {
        $inspection_date = sprintf('2026-02-%02d', $day);
...
echo "\n====================================================\n";
echo "CTP PU CASE FEBRUARY 2026 IMPORT COMPLETE SUMMARY:\n";
