<?php
// Importer for CTP PU Case Excel file for January 2026 (ctp_pucase_jan.xlsx) - Line REF 02
// Section: PU Case, starts after Sheet 3 (index >= 3)
require_once __DIR__ . '/../config/config.php';

echo "Starting CTP PU Case Import for January 2026 (ctp_pucase_jan.xlsx) - Line REF 02...\n";

$file = __DIR__ . '/../ctp_pucase_jan.xlsx';
if (!file_exists($file)) {
    die("Error: File ctp_pucase_jan.xlsx not found.\n");
}
...
for ($idx = $startIndex; $idx < count($sheets); $idx++) {
    ...
    if (!$sheetXml) {
        echo "\n----------------------------------------------------\n";
        echo "Skipping Sheet $sheetNum '$sheetName' (no XML)\n";
        continue;
    }
    ...
    // Model name = Sheet Name as requested
    $model_name = trim($sheetName);

    // Item check name: try Row 3 col L first (for RH/PH/RT/PT sheets), then Row 4 col L (for Reactivity/JIG sheets), fallback to sheet name
    $item_check_name = '';
    ...
    if (!$item_check_name) {
        $item_check_name = $model_name;
    }

    // LSL / USL from rows 8-9 col K (label) and M (value) - confirmed from inspection
    $lsl = null;
    $usl = null;
    for ($r = 5; $r <= 12; $r++) {
        ...
        if ($kVal === 'USL') {
            $usl = isset($rows[$r]['M']) && is_numeric($rows[$r]['M']) ? floatval($rows[$r]['M']) : null;
        }
        if ($kVal === 'LSL') {
            $lsl = isset($rows[$r]['M']) && is_numeric($rows[$r]['M']) ? floatval($rows[$r]['M']) : null;
        }
    }

    if ($lsl === null || $usl === null) {
        echo "\n----------------------------------------------------\n";
        echo "--> SKIPPED: Sheet $sheetNum '$sheetName' - Could not detect LSL/USL.\n";
    ...
    // Detect date header row (rows 10..20, look for date 1-31)
    $dateRow = null;
    for ($r = 10; $r <= 20; $r++) {
        if (isset($rows[$r])) {
    ...
    if (!$dateRow) {
        echo "\n----------------------------------------------------\n";
        echo "--> SKIPPED: Sheet $sheetNum '$sheetName' - No date row found.\n";
        $summary_results[$sheetName] = ['model' => $model_name, 'status' => 'SKIPPED_NO_DATA', 'sessions' => 0, 'measurements' => 0];
        continue;
    }

    // Find sample rows below dateRow (Column B = time label)
    $sample_row_indices = [];
    ...
    // Detect valid day columns with actual data
    $has_data = false;
    $valid_columns = [];
    ...
    echo "Model: '$model_name' | Item: '$item_check_name' | LSL: $lsl | USL: $usl $uom | DateRow: $dateRow | TimeSlotsFound: " . count($sample_row_indices) . " | DataCols: " . count($valid_columns) . "\n";
    ...
    $stmt_spec_find->execute([
        ':model' => $model_name, ':item' => $item_check_name, ':proc' => $process_name,
        ':line' => $line_name, ':sec' => $section_name, ':dt' => $data_type,
        ':lsl' => $lsl, ':usl' => $usl
    ]);
    $spec_id = $stmt_spec_find->fetchColumn();

    if (!$spec_id) {
        $conn->prepare("INSERT INTO dtc_master_dtc_specs (
            model_name, item_check_name, data_type, line_name, section_name, process_name,
            measuring_item, lsl, usl, target_value, uom, target_zst, target_zlt
        ) VALUES (
            :model_name, :item_check_name, :data_type, :line_name, :section_name, :process_name,
            :measuring_item, :lsl, :usl, :target_value, :uom, 4.0, 3.0
        )")->execute([
            ':model_name' => $model_name, ':item_check_name' => $item_check_name,
            ':data_type' => $data_type, ':line_name' => $line_name, ':section_name' => $section_name,
            ':process_name' => $process_name, ':measuring_item' => $measuring_item,
            ':lsl' => $lsl, ':usl' => $usl, ':target_value' => $target_val, ':uom' => $uom
        ]);
    ...
    // 2. Find or Insert Parameter for target_month
    ...
    if (!$parameter_id) {
        $conn->prepare("INSERT INTO dtc_master_parameters 
            ...
            ':spec_id' => $spec_id, ':target_month' => $target_month,
            ':item_check_name' => $item_check_name, ':data_type' => $data_type,
            ':line_name' => $line_name, ':section_name' => $section_name,
            ':process_name' => $process_name, ':measuring_item' => $measuring_item
        ]);
    ...
    foreach ($valid_columns as $col => $day) {
        $inspection_date = sprintf('2026-01-%02d', $day);
        ...
        $stmt_ins_m = $conn->prepare("INSERT INTO dtc_measurements (session_id, sample_sequence, sample_label, sample_value, created_by, modified_by, modified_date)
            VALUES (:sid, :seq, :lbl, :val, :cb, :mb, CURRENT_TIMESTAMP)");
        for ($i = 0; $i < count($sample_inputs); $i++) {
            $raw_val = $sample_inputs[$i] ?? '';
            if ($raw_val !== '') {
                $stmt_ins_m->execute([
                    ':sid' => $session_id, ':seq' => $i + 1,
                    ':lbl' => $sample_labels[$i] ?? "Sample " . ($i + 1),
                    ':val' => $raw_val, ':cb' => $operator_id, ':mb' => $operator_id
                ]);
    ...
    $summary_results[$sheetName] = [
        'model' => $model_name, 'item' => $item_check_name, 'spec_id' => $spec_id,
        'parameter_id' => $parameter_id, 'status' => 'IMPORTED',
        'lsl' => $lsl, 'usl' => $usl,
        'sessions' => $sessions_created, 'measurements' => $measurements_created
    ];
}
...
echo "\n====================================================\n";
echo "CTP PU CASE JAN 2026 LINE REF 02 IMPORT COMPLETE:\n";
...
echo "Sheets imported: " . count($imported) . "\n";
