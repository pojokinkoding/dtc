...
$filesToImport = [
    [
        'file' => '202602 Time_Check_H press out_door REF01.xlsx',
        'month' => '2026-02'
    ],
    [
        'file' => '202603 TIME CHECK H PRESS OUT DOOR REF01.xlsx',
        'month' => '2026-03'
    ],
    [
        'file' => '202604 TIME CHECK H PRESS OUT DOOR REF01.xlsx',
        'month' => '2026-04'
    ],
    [
        'file' => '202605 TIME CHECK H PREESS OUT DOOR REF01.xlsx',
        'month' => '2026-05'
    ],
    [
        'file' => '202606 TIME CHECK H PRESS REF01.xlsx',
        'month' => '2026-06'
    ]
];

echo "=======================================================\n";
echo "=== BATCH IMPORT H PRESS OUTDOOR (2026-02 to 2026-06) ===\n";
echo "=======================================================\n\n";
...
echo "=== ENSURING ALL DAYS ARE CLOSED FOR 2026-02 TO 2026-06 ===\n";
$conn = getDBConnection();
$operator_id = $conn->query("SELECT user_id FROM dtc_users WHERE role = 'Admin' ORDER BY user_id ASC LIMIT 1")->fetchColumn() ?: 1;

$stmt_params = $conn->prepare("SELECT parameter_id, line_name, section_name, model_name FROM dtc_master_parameters WHERE target_month = :tm AND section_name = 'H Press Out Door'");
...
foreach ($filesToImport as $item) {
    ...
    $pList = $stmt_params->fetchAll(PDO::FETCH_ASSOC);
    ...
    echo "Month $mStr: Closed all $daysInMonth days for " . count($pList) . " models. (Created missing closed sessions: $created, Updated open sessions: $closed)\n";
}

echo "\n=======================================================\n";
echo "=== BATCH IMPORT COMPLETE VERIFICATION ===\n";
echo "=======================================================\n";
